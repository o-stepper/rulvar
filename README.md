...
minimal line printer). Eighteen package names ship in total (seventeen packages plus the unscoped
pointer), sixteen in lockstep at a single version. Full map and dependency graph:
[reference/packages](https://docs.rulvar.com/reference/packages).

<details>
<summary>All eighteen package names</summary>
...
- [Orchestration modes](https://docs.rulvar.com/guide/orchestration-modes): the dynamic
  orchestrator's acceptance machinery, the claim consistency pass, the citation entailment
  audit (sampled or as a full census) with the anchor grounding lint, and
  [production profiles](https://docs.rulvar.com/guide/production-profiles), which compile
  the regulated fail-closed floor in one call
- [Tools](https://docs.rulvar.com/guide/tools), [MCP](https://docs.rulvar.com/guide/mcp),
  [stores](https://docs.rulvar.com/guide/stores),
  [evals](https://docs.rulvar.com/guide/evals) (graders, the benchmark kit, the contract
  audit lexer),
  [CLI](https://docs.rulvar.com/guide/cli) (run, resume, and replay, portable across
  checkouts via `--registry`),
