...
interface ToolExecutorRegulatedPosture {
  ...
  */
  isolation: {
    ...
    flavor: "container";
    network: string;
    readOnlyRoot: boolean; /** The image reference; the regulated floor requires a `@sha256:` digest. */
    image: string; /** The `--cap-drop` list; the regulated floor requires `ALL` among them. */
    capDrop: readonly string[]; /** The `--memory` value. */
    memory: string; /** The `--cpus` value. */
    cpus: string; /** The `--pids-limit` value. */
    pidsLimit: number; /** Where the work directory is mounted inside the container. */
    workMount: string; /** Where the ephemeral scratch directory is mounted beside a worktree (RV4914). */
    scratchMount: string; /** The raw extra `docker run` flags, verbatim; the regulated floor refuses any. */
...
//#endregion
//#region src/runtime/evidence-categories.d.ts
/**
...
declare const EVIDENCE_CATEGORIES: readonly EvidenceCategory[];
/**
...
*/
declare function classifyEvidenceFile(file: string): EvidenceCategory;
//#endregion
...
interface AgentResult<T> {
  ...
  */
  evidence?: {
    recordedEntries: number;
    minEntries: number;
    met: boolean; /** The per category verdict (RV4908), present when the contract declared a distribution. */
    byCategory?: Partial<Record<EvidenceCategory, {
      recorded: number;
      required: number;
    }>>;
  };
  ...
  */
  evidenceFloor?: {
    recordedEntries: number;
    minEntries: number; /** The per category shortfall (RV4908), present when a distribution was declared. */
    byCategory?: Partial<Record<EvidenceCategory, {
      recorded: number;
      required: number;
    }>>;
...
interface RunAgentOptions<S extends SchemaSpec = JsonSchema> {
  ...
  */
  evidenceContract?: {
    minEntries: number;
    enforce?: "warn" | "refuse"; /** The required spread over source categories (RV4908); see the ctx contract. */
    distribution?: Partial<Record<EvidenceCategory, number>>; /** Replaces the default path classifier (RV4908). */
...
*/
declare const CITATION_VERDICT_EST_TOKENS_PER_ROW = 70;
/** The bijection's fixed frame beside the rows (RV4706): array, envelope, preamble. */
declare const CITATION_VERDICT_EST_BASE_TOKENS = 500;
/**
...
interface AcceptanceChildSummary {
  ...
  */
  ...
  evidence?: {
    ...
    */
    byCategory?: Partial<Record<string, {
      recorded: number;
      required: number;
    }>>;
  };
...
*/
declare const RESEARCH_FAN_OUT_LIMITS: UsageLimits;
/** Options of {@link researchFanOut}: the research template's plus the money and the roster. */
interface ResearchFanOutOptions extends ResearchAgentProfileOptions {
  /**
...
interface ResearchFanOutResult extends ResearchAgentProfileResult {
...
*/
declare function researchFanOut(options: ResearchFanOutOptions): ResearchFanOutResult;
...
*/
declare const CITATION_UNIT_JUDGE_EXTENSION_FACTOR = 2;
/**
...
*/
declare function citationUnitExcerptOf(resolve: (target: CitationTarget) => string | undefined, row: Pick<CitationAuditRow, "path" | "line" | "endLine">, caps?: {
  maxLines?: number;
  maxChars?: number;
}): {
  excerpt: string;
