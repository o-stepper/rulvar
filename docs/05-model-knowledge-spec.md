...
Profile evidence section (amended 2026-07-11; found by the M12 checkpoint, whose criterion 2 measured a card carrying no agentType-actionable signal). FR-607 commits the card to feeding agentType choice at spawn in the profileCard vocabulary, and tier-relative rows alone cannot do that, so the render gains one section between the two layers, projected onto the advertised spawn vocabulary. For each advertised profile pinning a concrete string ModelRef (ladder declarers and model-less profiles do not participate), the eval-measured claims whose subject model equals the profile's model (restricted to the profile's effort when it pins one, all efforts otherwise) fold per taskClass with weakness winning over strength across efforts (the conservative fold), rendering `- <profile>: strong <classes>; weak <classes>` plus one fixed spawn-guidance line ("prefer the cheapest profile marked strong for the task at hand; avoid profiles marked weak at it"). The section renders ONLY when at least one profile line exists, so every card produced before this amendment, and every card without concrete-profile matches, stays byte-identical (recorded journals and cassettes unaffected). Profile names are the sanctioned vocabulary; model names still never render; human-editorial claims never enter the section.
...
Checkpoint decision (M12-T01, 2026-07-11; remeasured at the 2026-07-12 reopening): FAILED both times under the standing rule. Final decision (2026-07-12, the fresh verdict run under the founder-amended OQ-09 rule): PASSED, and criterion 2 cleared the ORIGINAL 105 percent branch as well (100.0 against 70.0 percent at 104.8 percent of baseline cost); criterion 1 held in all six fully measured runs. Phase 3 is UNBLOCKED: M12-T02 through M12-T04 may start. Criterion 1 (rung selection) held: the compiled verified layer reproduced baseline quality at a third of the cost on every cell it recommended. Criterion 2 (agentType selection) failed the 105 percent cost bar: the card-informed orchestrator matched the no-card baseline pass rate at 115 percent of its cost. Phase 3 therefore remains unshipped and M12 stays closed and unversioned; the full measured record, the instrument defects fixed en route, and the reopen trigger live at OQ-09 (14-open-questions.md).
