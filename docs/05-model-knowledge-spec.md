...
| Phase 3 | M12 (version unassigned, gated) | kb_propose; the modelObservations section; the inbox via LedgerExport; the human gate with attribution attestation. Ships ONLY after the measured-value checkpoint of phases 1-2: the card demonstrably improves tier and agentType selection on eval cases. The quantitative criteria of that checkpoint are an open question (14-open-questions.md). |
