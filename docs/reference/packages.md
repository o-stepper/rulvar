...
Rulvar ships as fourteen packages from a single monorepo: thirteen under the `@rulvar` npm scope, plus `eslint-plugin-rulvar`, which follows the ESLint plugin naming convention. The packages release lockstep at one version, currently <!-- version:lockstep -->1.58.0<!-- /version -->, with a single exemption: `@rulvar/compat` is versioned independently and currently sits at <!-- version:compat -->0.1.1<!-- /version -->. Every package is ESM only, requires Node.js >= 22.12.0, and is licensed Apache-2.0.
...
All packages, including `eslint-plugin-rulvar` despite its unscoped name, release together at one version; the current release is <!-- version:lockstep -->1.58.0<!-- /version -->. Lockstep buys a simple compatibility rule: a set of `@rulvar` packages at the same version is the tested configuration. When you upgrade, upgrade them together. The unscoped `rulvar` pointer tracks the umbrella's version.
