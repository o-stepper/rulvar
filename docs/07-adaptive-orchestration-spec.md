...
- Ladder-declaring profiles are declaration-only under plain `spawn_agent` (amended 2026-07-11, found live by the M12 checkpoint): rung execution is owned by the plan extension, which resolves each attempt to a concrete per-attempt model override, so a plain spawn of a declarer can only die at wire resolution (04-model-layer-spec.md, section 12). The orchestrator therefore (1) excludes declarers from the spawn vocabulary in the profile card, listing their names on a separate "Declared ladders" context line (they still anchor the knowledge card's tier coordinates and still reach the extension IO in full), and (2) rejects a `spawn_agent` naming a declarer with a typed ConfigError BEFORE admission, so no admission slot or journal entries are burned. Without this split, a knowledge card praising ladder tiers steers the model into doomed spawns, and the card-informed run measures WORSE than the uninformed one, which is exactly what checkpoint run 5 observed (criterion 2: 50 percent informed versus 90 percent baseline).
...
- The orchestrator agent's own admission reserve (amended 2026-07-12, found live by the M12 checkpoint): a capped orchestrator dispatches with `estCost = effectiveCapUsd`, and the forced-finish agent with `estCost = finalizeReserveUsd`, because layer 2 makes those the true worst cases. Without the hints the default reserve prices the model's FULL `maxOutputTokens` (about one dollar on strong tiers), the commitment rides the whole ancestor chain for the lifetime of the orchestrator agent, small run ceilings sit at zero admission remainder throughout, and EVERY child spawn dies with a `budget` rejection: the checkpoint's orchestrated arms never admitted a single child under their case ceilings, so both A/B arms measured a self-solving orchestrator instead of agentType selection.
