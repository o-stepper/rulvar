...
| <a id="property-ttlms"></a> `ttlMs?` | `number` | The store's lease ttl; the worker renews at ttl/3 (the normative bound, docs/03 12.3). Default: the Appendix A reference 60000 ms. MUST match the store's configured ttl. | [packages/cli/src/worker.ts:72](https://github.com/o-stepper/rulvar/blob/main/packages/cli/src/worker.ts#L72) |
