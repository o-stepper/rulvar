...
| ------ | ------ | ------ | ------ |
| <a id="property-now"></a> `now?` | () => `number` | Injectable clock for lease-expiry tests. | [packages/store-sqlite/src/store.ts:109](https://github.com/o-stepper/rulvar/blob/main/packages/store-sqlite/src/store.ts#L109) |
| <a id="property-path"></a> `path` | `string` | Database file path, or ':memory:' for an in-process store. | [packages/store-sqlite/src/store.ts:98](https://github.com/o-stepper/rulvar/blob/main/packages/store-sqlite/src/store.ts#L98) |
| <a id="property-ttlms"></a> `ttlMs?` | `number` | Lease ttl; default the Appendix A interim reference (60000 ms). An integer between 1 and 2147483647 ms (workers renew on Node timers at ttl/3), refused as a ConfigError BEFORE the database opens: zero or a negative made every lease born expired (a second owner could take over immediately), NaN failed the first acquire with a raw sqlite NOT NULL error, and Infinity never expired (v1.35.0 review P2-4). | [packages/store-sqlite/src/store.ts:107](https://github.com/o-stepper/rulvar/blob/main/packages/store-sqlite/src/store.ts#L107) |
