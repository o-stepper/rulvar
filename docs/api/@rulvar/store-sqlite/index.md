...
| [QUOTA\_BUSY\_TIMEOUT\_MS](/api/@rulvar/store-sqlite/variables/QUOTA_BUSY_TIMEOUT_MS.md) | How long a runtime reserve/reconcile transaction waits for a sibling process's transaction before the driver reports busy. Quota admissions are short single-writer transactions; queueing here IS the cross-process serialization working. |
