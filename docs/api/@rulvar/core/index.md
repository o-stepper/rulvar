...
| [FailRunError](/api/@rulvar/core/classes/FailRunError.md) | A declared fail-run policy engaged and closed the run as a failure (v1.35.0 review P2-1): `budget.atCap: 'fail-run'` after the journaled orchestrator cap decision, or `guards.fallback: 'fail-run'` after the journaled guard verdict. The run outcome is 'error' with this code; `data.source` names the policy ('orchestrator_budget_cap' or 'plan_guards') and `data` carries the decision entry reference, so the outcome is a pure roll forward of the journal on resume: no second decision, no model call, no spend. |
