...
| ------ | ------ | ------ | ------ |
| <a id="property-args"></a> `args?` | `unknown` | The run's original arguments: not journaled for in-process workflows in v1, so the host supplies them (resume binding residuals, docs/14). | [packages/core/src/engine/engine.ts:216](https://github.com/o-stepper/rulvar/blob/main/packages/core/src/engine/engine.ts#L216) |
...
| <a id="property-invalidate"></a> `invalidate?` | `number`[] | invalidate/retry: entries to unpin before matching (docs/03, section 6.5). | [packages/core/src/engine/engine.ts:224](https://github.com/o-stepper/rulvar/blob/main/packages/core/src/engine/engine.ts#L224) |
