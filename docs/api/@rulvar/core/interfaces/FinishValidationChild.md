...
| <a id="property-salvageableoutput"></a> `salvageableOutput?` | `readonly` | `boolean` | Present and true ONLY when acceptance.acceptValidatedTerminalOutputOnLimit is configured and this child settled 'limit' CARRYING a terminal output (the finalization reserve summary that, for a schema child, already validated against the declared output schema). Acceptance will count such a child as a success, so evidencePreservedValidator treats its text as part of the cited evidence pool. Absent in every other configuration, keeping the old pool exactly. | [packages/core/src/orchestrator/finish-validators.ts:44](https://github.com/o-stepper/rulvar/blob/main/packages/core/src/orchestrator/finish-validators.ts#L44) |
