...
| <a id="property-wireidsmissing"></a> `wireIdsMissing` | `number` | Wire requests with no recorded join key, across EVERY dispatch row (RV1410): a multi-wire row contributes the requests its id set left unnamed, and a single-wire row contributes its one request when neither `responseId` nor an id set names it. Failed requests count like any other: the provider may have billed them, and a statement line cannot be joined to a row that has no id either way. | [packages/core/src/engine/invoice.ts:198](https://github.com/o-stepper/rulvar/blob/main/packages/core/src/engine/invoice.ts#L198) |
