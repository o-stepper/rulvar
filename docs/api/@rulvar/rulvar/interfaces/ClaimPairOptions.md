...
| <a id="property-reportuncovered"></a> `reportUncovered?` | `boolean` | Collect the citing sentences the reported pairs left UNCOVERED (RV4202, the sixth comparison experiment): the coverage-armed repair round needs the sentences themselves for its prompt, not only their count, because "raise the coverage" is actionable to a composing model exactly when it can see which claims the pool never grounded. Distinct collapsed sentences, draft order, each cut to `maxExcerptChars`, capped at [MAX\_UNCOVERED\_SENTENCES](/api/@rulvar/rulvar/variables/MAX_UNCOVERED_SENTENCES.md); the uncapped count rides beside the list. Unset = byte-identical fold output. | `packages/core/dist/index.d.ts` |
