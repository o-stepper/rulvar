...
| <a id="property-rows"></a> `rows?` | [`AppliedPricingRow`](/api/@rulvar/rulvar/interfaces/AppliedPricingRow.md)[] | The pinned rows the fold used; present on snapshot-priced exports. Each row's `rates` carries `ratesVerifiedAt` when the pinning table stamped one (RV814): the machine-readable answer to how fresh the rates that priced settled history were. | `packages/core/dist/index.d.ts` |
