...
| <a id="property-byagenttype"></a> `byAgentType` | `Record`\&lt;`string`, `number`\&gt; | Spawn agentType names; absent and empty fold under 'unknown' (RV3604). | `packages/core/dist/index.d.ts` |
...
| <a id="property-byphase"></a> `byPhase` | `Record`\&lt;`string`, `number`\&gt; | ctx.phase names; phase is structural for this map. Spend with no phase, or an EMPTY phase, folds under the named 'unknown' bucket (RV3604): a '' key is unaddressable in every downstream table, and the third comparison run's report read `byPhase {"": 5.58}` for the whole run. | `packages/core/dist/index.d.ts` |
