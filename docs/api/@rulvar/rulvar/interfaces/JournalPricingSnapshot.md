...
| <a id="property-ratesverifiedat"></a> `ratesVerifiedAt?` | \{ `newest`: `string`; `oldest`: `string`; \} | The last pin's freshness range (RV3703); see the per-segment field. Absent when no row of the last pin is dated. | `packages/core/dist/index.d.ts` |
...
| <a id="property-rowshash"></a> `rowsHash` | `string` | The last pin's content hash (RV3703); see PinnedPricingSegment.rowsHash. | `packages/core/dist/index.d.ts` |
