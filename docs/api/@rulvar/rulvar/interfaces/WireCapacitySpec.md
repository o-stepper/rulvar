...
| <a id="property-maxsemanticrepairrounds"></a> `maxSemanticRepairRounds?` | `number` | Mirrors OrchestrateOptions.maxSemanticRepairRounds (RV4705): the scoped semantic reserve inside the pool. It shrinks the mechanical share of `repairWiresCeiling` exactly like the runtime split; greater than the declared total refuses typed, the intake contradiction. | - | `packages/core/dist/index.d.ts` |
