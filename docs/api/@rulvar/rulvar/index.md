...
| [LeasableStore](/api/@rulvar/rulvar/interfaces/LeasableStore.md) | Lease capability: acquire on a held lease MUST reject with a typed LeaseHeldError; renew MUST run at an interval of at most ttl/3; an append carrying a stale epoch MUST be rejected and never appear in load. |
...
| [ToolExecutor](/api/@rulvar/rulvar/type-aliases/ToolExecutor.md) | Where execute runs. A declared capability consumed by dispatch and policy; only 'inprocess' is enforced in v1, subprocess/container remain declared capability while the executor design stays an open question. |
...
| [requiredSectionsValidator](/api/@rulvar/rulvar/functions/requiredSectionsValidator.md) | Requires every named section to appear LITERALLY in the result text (a heading like 'FINDINGS' or any marker the goal demands). Default name 'required-sections'; pass `name` to run several instances. |
