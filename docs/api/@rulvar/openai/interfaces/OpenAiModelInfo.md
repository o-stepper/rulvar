...
| <a id="property-wiremaxeffort"></a> `wireMaxEffort` | `boolean` | The model accepts wire `reasoning.effort: "max"` (GPT-5.6 Sol per the official model docs). When false, canonical max downmaps to wire xhigh; the downmap is recorded in providerMetadata and the journal identity keeps max, so caps accept the full canonical set either way. | [packages/openai/src/caps.ts:23](https://github.com/o-stepper/rulvar/blob/main/packages/openai/src/caps.ts#L23) |
