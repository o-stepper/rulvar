...
| [runPlanned](/api/@rulvar/planner/functions/runPlanned.md) | plan-then-run in one call (amended during M6-T05: the composition is async because planning itself is a run). options.plan bounds the planning conversation, options.run bounds the generated workflow's execution; the two ceilings are independent, and the bare form without options runs BOTH legs unbounded, as before. |
