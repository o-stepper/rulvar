...
- Provider constraints the adapter MUST respect (and the router's tier selection must know): no recursive schemas, incompatible with citations (400), incompatible with assistant prefill.
- Constrained-decoding keyword subset (amended 2026-07-11, measured live on claude-sonnet-5 by the M12 checkpoint): the schema validator behind BOTH strict tools and `output_config.format` json_schema rejects `minimum`, `maximum`, `exclusiveMinimum`, `exclusiveMaximum`, `multipleOf` and (asymmetrically) `maxItems` with a 400 naming the offending type and keyword; `minItems`, `minLength`, `maxLength`, `pattern`, `format`, `enum` and `const` are accepted. Plain (non-strict) tools accept full JSON Schema. The adapter MUST scrub the rejected keywords from the wire copy of strict tool schemas and of every format schema, at schema positions only (a property literally named `minimum` survives), and MUST NOT mutate the engine-side schema: the engine keeps validating tool args and structured output against the unscrubbed original, so the constraint stays enforced and only the model-side hint is lost. Strict eligibility itself remains the shape rule of the previous bullet; carrying a scrubbed keyword never demotes a tool to non-strict. The earlier claim here that "numeric or string min-max constraints" are unsupported was half right: string bounds pass, and the adapter did not scrub at all, which is exactly how the orchestrator's spawn tools (integer `minimum` on `startTier` and handles) produced pre-first-call 400s in every live orchestrate run.
