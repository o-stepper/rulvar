...
- Compaction is owned by the Agent Runtime: history processors per profile plus a `contextWindow` threshold (default 0.8); compaction points are written into the checkpoint. Committed semantics (M4-T03): compaction is ON by default for every agent at threshold 0.8 of the loop model's `caps.contextWindow`; the context estimate is the last loop turn's `inputTokens + outputTokens` (the Usage invariant makes inputTokens the full prompt); the check runs at tool turn boundaries, BEFORE the boundary checkpoint, so a crash after compaction resumes compact. The summarize invocation runs through the resolution chain with role `summarize`; when no layer resolves a summarize model it falls back to the loop-resolved model (the low role-effort default still applies). The summary replaces everything after the first message as ONE user-role summary message; the projection rule applies to the summarize request itself, and a tool-bearing transcript carries the contracts with `toolChoice: 'none'`. A failed or empty summarize disables compaction for the rest of the run with a warning (budget and cancellation aborts still propagate); checkpoint `compaction` records the turn numbers at which compaction fired, restored verbatim on resume so a resumed run never re-summarizes already-compacted history.
...
| no-progress detector N | agent runtime (docs/14) | 3 consecutive turns without tool calls or artifact deltas (committed during M3 entry per the TBD rule; FROZEN as the 1.0 default at M9-T05, closing OQ-15: the per-run knob stays configurable, and richer heuristics are ordinary post-1.0 evolution behind it) | yes | journaled abort class |
| KB active-claims cap | ModelKnowledge (docs/05) | 8 per (model, taskClass) | yes | |
| renderBudget (WakeDigest, ledger render, KB card) | docs/07, docs/05 | measure: CHARACTERS, model-independent (OQ-04 closed at M10 entry). WakeDigest: 400 chars per outputSummary row, one value serving the distillation cap and the digest render default (adopting the implemented cap unchanged, frozen into every cassette since M6). ledger_read render: 65536 chars over the serialized view (over budget, auto-derived rows drop oldest-first: world delta, then task digests, then revision history; then authored sections drop oldest-first; the brief slices last; every drop is a FLAGGED discrepancy line). KB card: 4096 chars. (Committed during M10 entry per the TBD rule; initial values, M11 calibration against phase-1 telemetry closes OQ-08) | yes | per-orchestration renderBudgetChars overrides the digest render value; under the default termination limits the ledger render fits without truncation, so the belt engages only on raised limits; the section caps stay the primary bound |
