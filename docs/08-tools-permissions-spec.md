...
The seam question of OQ-18 closed at M9-T05 (2026-07-10): NO ExecutorProvider seam ships in L0 at 1.0, and the frozen seam list stays the six of 02-architecture.md, section "SPI seams and the 1.0 freeze". The subprocess and container executor specifications remain post-1.0 work; until they ship, all containment statements in this documentation set stay downgraded to plans and MUST NOT be presented as shipped behavior. Recorded direction, non-normative until that work lands: `subprocess` as a stdin/stdout JSON protocol with a timeout and `cwd` supplied by the IsolationProvider; `container` as a plugin behind a future ADDITIVE ExecutorProvider seam introduced under semver minor rules, never by widening a frozen seam.
