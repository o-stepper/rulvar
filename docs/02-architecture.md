...
Authentication is explicitly out of scope: the server is host-embedded and auth belongs to host middleware (14-open-questions.md). SSE reconnection uses Last-Event-ID mapped to the event seq (14-open-questions.md). Retention is opt-in (M8-T04; OQ-20 interim rule executed): the server accepts an optional `retention` predicate over RunMeta and applies `engine.deleteRun` to settled runs it selects.
