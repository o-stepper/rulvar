...
|---|---|---|---|---|
| R1 | MCP SDK v2 migration (v1 supported ~6 months after v2 ships) | M3 MCP bus rework | Pin `@modelcontextprotocol/sdk` ^1.29; the migration is the explicit post-M3 task M5-T10 (deferral, if v2 is not stable by M5, is recorded here with a new owning milestone no later than M8) | MCP SDK v2 stable release |
