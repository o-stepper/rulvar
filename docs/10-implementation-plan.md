...
|---|---|---|---|---|
| R1 | MCP SDK v2 migration (v1 supported ~6 months after v2 ships) | M3 MCP bus rework | Pin `@modelcontextprotocol/sdk` ^1.29; the migration is the explicit post-M3 task M5-T10. ASSESSED AND DEFERRED at M5-T10 (2026-07-07): no v2 line is published (npm `latest` is 1.29.0, the highest version; zero 2.x releases), so migration is not actionable. The MCP ToolSource stays on ^1.29 with the M3-T04 suite green and journal shapes unchanged; the migration re-owns to **M8** and fires only on the revisit trigger. RE-ASSESSED AND RE-DEFERRED at M8 entry (2026-07-09): still no v2 line (npm `latest` is 1.29.0, the highest version; zero 2.x releases), so migration remains not actionable; the ToolSource stays on ^1.29 unchanged and the migration re-owns to **M9** (the last pre-freeze milestone; the M9 SPI audit re-runs this assessment, and past 1.0 a v2 migration rides the ordinary deprecation policy), per the same rule. RE-ASSESSED at the M9-T05 SPI audit (2026-07-10): still no v2 line (npm `latest` is 1.29.0, the highest version; zero 2.x releases); the ToolSource seam froze on ^1.29 and a future v2 migration rides the post-1.0 deprecation policy as already provided. | MCP SDK v2 stable release |
