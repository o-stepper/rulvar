...
Since RV1907 the preflight report prices two minimums instead of leaving the operator to solve the wave by hand. `admission.requiredMinimumCeilingUsd` is the whole-wave fill: every declared row's reserve plus the finalize and synthesis carve-outs, the figure a viable `budgetUsd` must strictly exceed (children admit strictly below exact fill); the four-role benchmark's $6.00 ceiling sat $0.98 below its own wave's 6.98 and lost two of four mandated workers to it. `exposure.requiredMinimumExposureUsd` is the breathing floor of `maxInFlightExposureUsd`: the carve-outs plus the maxInFlight most expensive concurrent turn floors, the orchestrator's own turn among them; below it the coordinating turn beside a full child wave parks on the RV1902 wait, and the recovery arm's $3.20 cap stalled exactly there. As a worked shape: four workers at a 2,500 token output allowance and a 4,000 token orchestrator turn at $10 per million output tokens price the concurrent floors at about $0.14, so with a $1.00 synthesis reserve the exposure cap wants at least $1.14, while the ceiling wants strictly more than the reserves plus every admission row.
