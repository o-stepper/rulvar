...
- `scope` required (RV4007): a regulated run has an owner, recorded at genesis. The compile normalizes it (RV4107): the hash reads the normalized copy, junk fields cannot move it, and an empty or malformed scope refuses at compile time instead of run intake.
...
- Constructions the options reach must hold their posture (RV4101): an `mcp()` source must run `drift: 'refuse'` with every discovery bound declared, and a `bridgeAiSdk()` adapter must keep `providerExecutedTools: 'deny'`. A construction exposing no `describeRegulatedPosture()` is counted into the hash as `unrecognized` rather than implied verified.
...
The returned `profileHash` is a sha256 over the enforced posture map, and the compile writes it into `run.configFingerprint` as `regulated:2:<hash>` (the `2` is the posture-map version: RV4101 changed the map's meaning by adding the construction key). The existing fingerprint machinery does the rest: genesis records it, and a resume asserting a different fingerprint refuses before ownership. No new meta surface, no engine branch: the compiled options are DATA, applied like any others.
