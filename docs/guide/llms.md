...
- Current release: v<!-- version:lockstep -->1.10.0<!-- /version -->, Apache-2.0. All `@rulvar/*` packages version in lockstep; the exceptions are `@rulvar/compat` (independent) and the unscoped `rulvar`, a pointer package that only re-exports the umbrella. [Versioning](/reference/versioning)
...
| Run settles `suspended` | Every in-flight branch waits on `ctx.awaitExternal` or a tool approval. Read `outcome.pending`, call `handle.resolveExternal(key, value)`, resume. |
