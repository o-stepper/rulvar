...
Inside the window a call outside the allowlist receives a typed error tool result naming the window (`guard: 'finalization-window'` on the `tool:end` event, same posture as the [exploration guard](#exploration-guards) denials: visible to the model, never terminal, consuming no budget or units), and the model is told once, via a plain user message (`Finalization window: ...`), to record its evidence and finish. The notice is rendered when it is delivered, at the batch boundary, from the counts that bind there (RV4901): the entry is detected call by call inside a batch, so a notice composed at the entry used to name a remaining count the rest of the batch had already spent and a deficit the batch's own `record_evidence` results had already closed; the tenth comparison experiment's security specialist read "7 of the reserved final 7 tool calls remain, record 4 more evidence entries first" with 3 calls left and its floor met, obeyed, and settled `limit` at the cap. The delivered notice now agrees with the tool budget notice flushed after it, prints no deficit line for a floor the batch closed, and a boundary grant that reopens the budget before the delivery keeps the entry's own text. The allowlist defaults to the tools priced at `toolUnits` cost `0` (the free bookkeeping tools such as `record_evidence`); an explicit `allow` replaces that default. The engine terminal tool is always admitted regardless, and the `escalate` tool is structurally exempt (it is intercepted before the window check), so the window can never wall off the exits.
...
One overrun inside the window used to be a terminal `limit` whatever the call was: in the tenth comparison experiment a specialist died at 36 of 36 on a single surplus `record_evidence` call, with nine entries over a floor of four and a finished report in hand, and the `all-ok` acceptance rejected the run for it. The opt-in `finalizationWindow.onSurplus: 'answer'` (RV4902) grants a soft landing exactly there: when the tool budget expires inside the window on a tail of allowlisted calls (the bookkeeping the window itself invited) while the declared evidence floor is already met (the batch's own results counted, or no floor declared), the tail is answered with typed skipped results marked `surplus: true`, the model is told once by a plain user message (`Finalization surplus: ...`) that it has one answer turn, and on that turn only the terminal tool (or a plain text answer without a terminal tool) completes; any further tool call ends the run at the limit exactly as before, and the turn is granted once per invocation, counted from the message window so a resumed segment grants the same single turn. An overrun with the floor still open, or on a call outside the allowlist, stays the historical limit. The live `toolBudget` snapshot reports `surplusAnswerTurn: true` when the turn was granted. The default `'limit'` keeps every byte; enabling the option changes recorded model requests, exactly like the window itself.
...
A turn cut while a tool call's arguments were still streaming is named as the cut it is (RV4904): both first class adapters deliver such arguments as the `{__unparsed}` wrapper, and when the turn's finish reason is `max-tokens` the error tool result carries, beside the unchanged schema line, a `truncation` sentence with the arithmetic the model needs to size its retry (the characters that arrived, the output tokens the turn produced and how many of them were reasoning, and the allowance the request declared), the `tool:end` event carries `errorCode: 'truncated-arguments'`, and a `log` warning repeats the sentence. For the terminal tool the full `AgentResult` counts these exchanges in `truncatedTerminalExchanges` (window derived, a subset of `schemaRejectedTerminalExchanges`, absent when zero). The tenth comparison experiment's coordinator lost its first finish exactly so, at 15000 output tokens of which 10061 were reasoning, read only "failed validation", and re paid the whole document under the same allowance; the opt-in `limits.repairTurnMaxOutputTokens` gives the granted repair turn (the turn that follows a rejected terminal exchange) its own output allowance, the budget clamp and the model maximum still applying on top, and the dynamic orchestrator, when its own `limits` declare the field, tells the coordinator both allowances in its prompt so the document is sized before it is written.
