...
### The window notice {#the-window-notice}

The notice is one plain user message, deterministic for its counts so a recorded conversation rebuilds byte identically on resume and replay:

```text
Finalization window: 2 of the reserved final 4 tool calls remain. Only finalization tools (and the terminal tool) may execute now; record your evidence and finish with what you have. This tail is reserved for your declared evidence floor: record 2 more evidence entries first (implementation: 2 more).
```

The dimension names the binding budget (`tool calls`, `tool units`, or `turns` under `finalizationTurns`); the floor sentence appears only while a widened reserve owes entries, and its parenthesis only under a declared distribution. The entry into the window is detected call by call inside a tool batch, but a user message cannot interleave a batch, so the notice waits for the boundary. Since RV4901 the queue holds the FACT of the entry (the counts the loop saw when it fired, which the journaled decision entry keeps exactly as before) and the text is rendered at the flush, after the batch's results joined the history: the binding dimension's live remaining calls, its live reserve, and the deficit over the history the batch just joined, so the notice agrees with the tool budget notice flushed right after it and prints no floor sentence for a floor the batch closed. When a boundary grant reopened the budget before the flush, so that no window binds at delivery, the entry snapshot is delivered instead, byte identical to what the loop always said, and an entry at a batch boundary whose batch recorded nothing reads exactly as before. The tenth comparison experiment's security specialist entered the window on the third call of a six call `record_evidence` batch and read, after the batch, `7 of the reserved final 7 tool calls remain` with `record 4 more evidence entries first`, while 3 calls remained and all six entries were recorded; it obeyed, the fourth surplus call died at the cap, and a child with nine entries over a floor of four and a finished report settled `limit`. Before 1.253.0 every window notice was composed at the entry and delivered stale after the batch; 1.253.0 renders it at delivery.

### The evidence spread inside the window {#the-evidence-spread-inside-the-window}

With `evidenceContract.distribution` declared (RV4908, [the recommended posture](#the-recommended-tool-budget-posture)), every count the window reads is the spread aware one, computed by one function over the message window so live and resumed segments agree. The recorded entries are the successful `record_evidence` results in the history, each classified by its `file` path with `classifyEvidenceFile`, in this order: a `.test.` or `.spec.` file name or a `test`, `tests` or `__tests__` directory is `tests`; an `examples` or `example` directory is `examples`; a `docs` or `doc` directory or a `.md`, `.mdx`, `.rst` or `.txt` file is `docs`; everything else is `implementation` (a declared `classify` replaces the rule). The deficit is the LARGER of the total gap below `minEntries` and the sum of the per category gaps, and the floor is met only when both are zero. That one number widens the reserve under `reserveForEvidenceDeficit`, sizes the RV809 proactive extension grant, gates [the surplus answer turn](#the-surplus-answer-turn), and appears in the notice, whose parenthesis lists the short categories (`implementation: 2 more, tests: 1 more`). The terminal verdict carries `evidence.byCategory` (`{ recorded, required }` per declared category) with `met` false while any category is short, and under `enforce: 'refuse'` the refusal's message and its journaled `evidenceFloor` name the short categories. Without a distribution every count is the plain total, byte for byte.

### The surplus answer turn {#the-surplus-answer-turn}

One overrun inside the window used to be a terminal `limit` whatever the call was: in the tenth comparison experiment a specialist died at 36 of 36 on a single surplus `record_evidence` call, with nine entries over a floor of four and a finished report in hand, and the `all-ok` acceptance rejected the run for it. The opt in `finalizationWindow.onSurplus: 'answer'` (RV4902) grants a soft landing exactly there, on four conditions read at the expiry: the tool budget expired inside the window; every call of the unexecuted tail is on the window allowlist (the bookkeeping the window itself invited); the declared evidence floor is met with the batch's own results counted, the spread included, or no contract is declared; and no answer turn was granted yet in this invocation, counted from the message window by the notice prefix so a resumed segment grants the same single turn. The tail is then answered with the typed skipped results (`skipped: true`, `surplus: true`), the boundary closes with the notice

```text
Finalization surplus: the tool budget expired inside the finalization window and 1 allowlisted call was skipped; your declared evidence floor is met. Answer now: call the 'finish' tool with your final result; any further tool call ends the run at the limit.
```

(`Answer now with your final text` when no terminal tool is declared), and on that turn only the terminal tool, or a plain text answer, completes; any further tool call ends the run at the limit exactly as before. An overrun with the floor still open, or on a call outside the allowlist, stays the historical limit. The live `toolBudget` snapshot reports `surplusAnswerTurn: true` when the turn was granted, and the child settles `ok`, so under an orchestrator it counts as an ordinary success rather than a salvaged one. The default `'limit'` keeps every byte; enabling the option changes recorded model requests, exactly like the window itself. Before 1.253.0 the overrun was a terminal `limit` with no option to soften it; 1.253.0 adds the opt in.
...
**Declare the spread the task wants.** A floor counts entries; it says nothing about where they come from. The tenth comparison experiment's task demanded citations across implementation, tests, documentation, and examples, the contract could say only `minEntries`, the specialists cited documentation alone, and the judge took the points. `evidenceContract.distribution` (RV4908) declares the required count per source category (`implementation`, `tests`, `docs`, `examples`); each recorded entry classifies by its `file` path (`classifyEvidenceFile`, whose exact order [the window section](#the-evidence-spread-inside-the-window) states; `classify` replaces the rule), and the effective floor becomes the larger of `minEntries` and the categories' sum. Every surface that read the floor reads the spread: the [finalization window](#the-finalization-window)'s deficit and widened reserve, the RV809 proactive grant, the surplus gate, the terminal verdict (`evidence.byCategory` with `met` false while any category is short), the `enforce: 'refuse'` refusal (its message and journaled `evidenceFloor` name the short categories), and the window notice (`record 2 more evidence entries first (implementation: 2 more)`). Preflight sizes `tool-cap-below-evidence-floor` to the effective floor. Absent, every surface keeps its bytes.
...
| `tool-cap-binds-before-budget`     | info     | a positive cap beside declared money and no extension; the cap binds first and the money stays unspent (RV4905) |
...
### Truncated terminal calls {#truncated-terminal-calls}

A turn cut while a tool call's arguments were still streaming is named as the cut it is (RV4904). Both first class adapters deliver such arguments as the `{__unparsed}` wrapper; when the turn's finish reason is `max-tokens` and the recovery pass cannot restore them, the error tool result keeps its schema line byte for byte (`arguments for 'finish' failed validation`, the line the window derived counters read) and adds a `truncation` sentence with the arithmetic the model needs to size its retry:

```text
the turn ended at its output token allowance (finish reason 'max-tokens') before the arguments for 'finish' closed: N characters arrived; the turn produced 15000 output tokens (10061 of them reasoning) against an allowance of 15000; shorten the result, split it across calls, lower the reasoning effort, or raise limits.maxOutputTokensPerTurn (https://docs.rulvar.com/guide/agents#output-truncation)
```

The reasoning parenthesis appears when the adapter reported reasoning tokens, and a request that declared no allowance reads `against the adapter default allowance`. The `tool:end` event carries `errorCode: 'truncated-arguments'` (an ordinary unparsed rejection keeps `invalid-arguments` and its exact bytes), a `log` warning repeats the sentence, and for the terminal tool the full `AgentResult` counts these exchanges in `truncatedTerminalExchanges`, window derived like `schemaRejectedTerminalExchanges` and a subset of it, absent when zero. The tenth comparison experiment's coordinator lost its first finish exactly so, under a 15000 token allowance of which 10061 went to reasoning, read only `failed validation`, and re paid the whole document under the same allowance. The opt in `limits.repairTurnMaxOutputTokens` gives the granted repair turn (the turn that follows a rejected terminal exchange, dispatched as phase `repair`) its own output allowance, the budget clamp and the model maximum still applying on top, while every other turn keeps `maxOutputTokensPerTurn`; and the dynamic orchestrator, when its own `limits` declare the field, tells the coordinator both allowances in its prompt: `Each of your turns is cut at 15000 output tokens, reasoning included; a rejected finish gets one repair turn with 40000 output tokens. Size finish({ result }) to fit the allowance, and split a long document across sections when it cannot.` (`cut at the model's default output allowance` when `maxOutputTokensPerTurn` is undeclared). Before 1.253.0 the cut read as a schema failure and the repair turn kept the cut's own allowance; 1.253.0 closes both.
