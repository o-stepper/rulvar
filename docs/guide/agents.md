...
Each grant is announced to the model as a plain user message with the exact new counts (`Tool budget extended: grant 1 of 3; ...`), flushed with the budget notices after the batch's results, and a `log` info event names the new cap. The extension raises `maxToolCalls` only, never `toolUnits`, and a terminal `finish` never spends a grant: it already rides the [budget exemption](#the-agent-loop-and-turns). On resume the grants re-derive conservatively from the restored executed-call count alone (calls beyond the base cap can only have been admitted by grants), so nothing new is journaled or checkpointed. An invocation without the field behaves byte-identically to before; enabling it changes the requests a recorded cassette would match, exactly like `toolBudgetNotices`. Preflight adds two findings: `inert-tool-budget-extension` (warning) for an extension with no `maxToolCalls` to extend, and `tool-budget-extension-exposure` (info) naming the worst-case extra calls.

Whenever `maxToolCalls`, `toolUnits`, or the extension is configured, the full `AgentResult` (and the live `agent:end` event, and the invocation table's agent rows) carries the `toolBudget` pressure snapshot: `{ used, cap?, unitsUsed?, unitsMax?, extensionsGranted?, noticesFired?, finalizationReserveUsed?, finalizationWindowEntered?, limiter? }`, with `cap` the effective cap after grants and `limiter` present only on a tool-budget `limit`. It is live telemetry only, exactly like `transportRetries`: never journaled, absent on a replayed result, and the way a host sees cap pressure before a starved worker ever settles `limit`. That boundary is deliberate, and it does not weaken resume: everything the runtime DECIDES from the snapshot re-derives conservatively from restored state (grants from the restored executed-call count, the finalization window from the same counts the guards rebuild), so a resumed run behaves identically whether or not the live telemetry was ever observed. What the boundary does mean is that the snapshot itself is gone once the process is: a host that wants budget pressure in a durable audit trail must export the live `agent:end` events (or the [invocation table](/guide/observability#the-invocation-model) built from them) into its own telemetry sink as they happen, exactly as it already must for `transportRetries`.
