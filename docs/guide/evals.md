...
**One question, one contract.** Both arms receive the same task verbatim and the same declared output contract (structure, coverage, length window). On the Rulvar arm the contract is enforced by the [finish validators](/guide/orchestration-modes#the-synthesis-invocation); on the other arm it is at least measurable after the fact. Contract compliance is scored mechanically per arm before any judge reads any prose.
