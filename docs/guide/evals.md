...
- `validator-guidance-conflict` (RV2202, RV2210): the c3 trap finish (a live-observed claim with no artifact in its sentence) repairs in ONE round because the evidence-grade reason names the composition that cannot trip its cited-value sibling, and the scenario asserts the guidance bytes actually rode the repair exchange; the third subscription run burned both repairs between the two verdicts.
