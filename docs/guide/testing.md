...
- **`capsOverrides`** (on `FakeAdapter` directly) layers declared capability fields over the fake defaults, so an offline test can drive caps-driven runtime behavior: `new FakeAdapter({ agents, capsOverrides: { minOutputTokensPerTurn: 16 } })` exercises the [provider output floor](/guide/budgets#layer-2-the-per-turn-guard-and-the-output-bound) exactly as a live OpenAI adapter would declare it.
