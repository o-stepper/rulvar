...
The bound itself is validated, not clamped: `attempts` must be an integer from 1 to `MAX_LIVE_SMOKE_ATTEMPTS` (10) and `baseDelayMs` a non-negative integer, and anything else, `NaN`, `Infinity`, and fractions included, rejects with a typed `ConfigError` before any stream opens. A helper whose whole contract is a bounded spend refuses configurations that are not.
