...
| External effects | The DESIGN: state machines, the journaled consumption protocol, the capability matrix, and the kill point catalog in the effects RFC (architecture below); runtime is plan 44 scope | Today: every effect path, transactional outbox, provider integration, and reconciliation; after plan 44: the provider adapters, IAM, thresholds, and runbooks the RFC assigns to the host | The RFC's promotion checklist (below), answered in writing per provider |
