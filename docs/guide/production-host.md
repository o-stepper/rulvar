...
| The assurance floor | `compileRegulatedProfile` v5: refuses loosening by field name, requires the deliverable contract, fills the production resolver and persistence, forces the advisory hook allow and hashes every permission layer (RV4911), hashes the posture into `regulated:5:<profileHash>` | The validators (the host's own acceptance criteria), the citation snapshot resolver, the judge routing, and the budget ceilings | The compiled `profileHash` pinned in the deployment config, and a red test proving a loosened posture refuses |
...
The regulated floor v5 (RV4303, RV4911), compiled from the host's own contract; the returned fingerprint reads `regulated:5:<profileHash>` and the resume assertion machinery pins it:
