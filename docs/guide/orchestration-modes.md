...
What the pass deliberately does NOT do: it never re-reads the source (that is `citedValueValidator`'s question), it never judges prose without a shared anchor, and it judges the DRAFT, so a `'carry'` synthesis that introduces a fresh inversion after the verdict is out of its reach; re-judging the synthesis output is a documented follow-up, not a shipped promise.
