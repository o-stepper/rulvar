...
Admission caps exactly two things (RV4909): the reserved wires per level, and, on any level configured with `concurrency`, the number of active grants (a tenant's active runs across its provider accounts, one provider account, one full scope). The reservation's `inputTokens`, `usd`, and `exposureUsd` ride the ticket for your own accounting and are never limited by admission: money is bounded by the [budget](/guide/budgets), not by the scheduler. A lease that expires under a live run refunds the wires the fenced cover proves unused and PARKS the run's concurrency slot (RV4910): the slot returns only through the run's own settle release, its cancel, its re admission under the same identity on resume, or an operator `cancel(unitId, generation, opId)` on the expired ticket, never by expiry alone, because expiry proves nothing about whether the holder is alive. What the run itself does with a lost lease is `admission.onLeaseLost`: `'continue'` (the default) announces `admission:lease-lost` once and lets the run go on under the wire quota; `'cancel'` verifies the grant on every renew tick and cancels the run when the scheduler no longer answers `granted`, so a hard cap deployment never runs work past its grant.
