...
- Offline resolution appends (a server writing an approval into a run it does not drive) take a brief lease around the append, exactly like `rulvar runs audit --repair` takes one around a repair.
- A dry-run preview (`ResumeOptions.dryRun`) performs zero store mutations and never acquires, so it stays available while another owner drives the run.
- `createEngine({ ownership: 'none' })` opts an engine out of automatic acquisition for hosts that coordinate ownership entirely outside the engine; a caller-supplied lease always wins over both modes.
...
### What admission caps, and what a lost lease does {#admission-caps-and-lost-leases}

Admission caps two things and nothing else (RV4909): the reserved wires per level (`capWires`, which is also the feasibility bound: a reservation no matched bucket could ever fit refuses as the terminal `denied` verdict at enqueue), and, on any level configured with `concurrency`, the number of active grants (a tenant's active runs across its provider accounts, one provider account, one full scope: the semaphore is taken at grant and restored at release on every level that declares it). The reservation's `inputTokens`, `usd` and `exposureUsd` ride the ticket for your own accounting and are never limited by admission, nor judged for feasibility: money is bounded by the [budget](/guide/budgets), not by the scheduler.

A lease that expires under a live run settles conservatively in two halves. The wires the fenced cover proves unused are refunded to every level, and the run's concurrency slot is PARKED (RV4910): the bucket's `held` count keeps counting it, the bucket's `parked` counter names it for operators, and the ticket is marked `slotParked: true` in the scheduler's persisted `AdmissionState` document (the sqlite and postgres schedulers persist exactly that shape, so the slot stays parked across a scheduler reopen, and a document written before RV4910 hydrates with nothing parked). The slot returns through one transition and never twice: the holder's own settle release, its cancel, its fresh enqueue under the same identity (the bracket's requeue on resume), or an operator `cancel(unitId, generation, opId)` on the expired ticket, never by expiry alone, because expiry proves nothing about whether the holder is alive. Before 1.253.0 expiry restored the slot at once and the next waiter took it under a possibly live holder (the Codex review of 973add91 that the owner confirmed found it); 1.253.0 parks it.

What the run itself does with a lost lease is `admission.onLeaseLost` on `createEngine`, validated at construction (`'continue'` or `'cancel'`; anything else is a `ConfigError`). Under the default `'continue'` the renew timer keeps every byte: a renew that throws logs `durable admission lease renew failed for run '<runId>': <error>; the scheduler may expire the lease and re-grant the capacity` at `warn` once per failure streak and then verifies the grant by `recover`; a grant the scheduler no longer holds emits `admission:lease-lost` once (with `unitId` and `generation`) beside a `warn` log saying the wire quota still gates every dispatch, and the run goes on; a renew that succeeds again logs `durable admission lease renew recovered for run '<runId>'` at `info`; and the settle release returns the parked slot. Under `'cancel'` every renew tick verifies by `recover`, thrown renew or not (the reference scheduler and the durable documents over it renew an expired ticket as a silent no op, so a hard cap deployment cannot wait for a throw), and a grant no longer held emits the same `admission:lease-lost` event, logs at `warn` that the run is cancelled instead of running past its grant, and cancels the run through its own cancellation machinery, so it settles `cancelled` and the settle release returns the slot. Admission stays environmental in both postures: nothing of it is journaled, and a resume never replays it.
