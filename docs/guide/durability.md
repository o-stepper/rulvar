...
Know the exact fencing boundary. What the epoch fences is every JOURNAL append of a leased run: engine appends under `ResumeOptions.lease`, queue worker appends, and the CLI server's resolution appends (the offline path acquires a lease and carries it, so a process that stalled past its ttl is refused instead of racing the new owner). `RunMeta` writes (`putMeta`) and transcript blobs are NOT fenced. The journal itself stays correct behind the epoch, but a superseded segment that has not yet noticed losing its lease can still write those two surfaces, and the worst case is worse than briefly stale catalog metadata. Its terminal `putMeta` can overwrite the successor's meta row with a terminal status and a regressed `segments` counter; if the successor then crashes before its own settle write, the run looks settled to every worker sweep and sits stranded until an operator resumes it by runId. And because turn checkpoints live at a deterministic ref derived from the dispatch seq, a stale segment still finishing a turn of the same attempt can overwrite the successor's checkpoint blob, so a later boot of that attempt (a crash resume, park or unpark, a dangling redispatch) can regress to older turn state: paid turns replay and the at-least-once tool window widens. Plan multi-writer deployments around that boundary rather than assuming every byte is fenced, and treat prompt worker shutdown on lease loss as load-bearing. The [fenced run state RFC](/contributing/rfc-fenced-run-state) tracks closing the gap with an opt-in fenced-writes store capability.
