...
`GET /runs/:id` answers for a run this process never held too, and since RV1209 it answers with the same shape: the response carries the terminal `envelope` rebuilt from the journal and marked `provenance: 'journal'`, priced through the same composed settle pins as the cost endpoint, or a typed `terminalUnavailable: { reason, message }` where nothing durable records a terminal. What a rebuilt envelope cannot carry, and why its absent `completion` and `error` are honest rather than empty, is in [the terminal envelope](/guide/observability#the-terminal-envelope).
