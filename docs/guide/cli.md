...
- `--json` (`invoice` only) prints the machine-readable `InvoiceExport` object instead of the line form, for piping into finance tooling. The export is self-describing: `pricingBasis` and `rowUsdNonAdditive` declare that per-row `usd` prices each call individually and need not sum to the total, and the additive per-row `allocatedUsd` column is the one whose flat sum reproduces the gross total exactly.
