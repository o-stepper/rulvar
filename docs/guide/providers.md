...
Verifying the price tables is a stamped, scheduled act (RV4918). Each adapter stamps its own table: `@rulvar/anthropic` carries one table wide constant, `RATES_VERIFIED_AT`, written into every priced row's `ratesVerifiedAt`, because the whole table mirrors one page snapshot; `@rulvar/openai` stamps each row, because every model has its own documented page. The scheduled audit (`scripts/rates-audit.mjs`, the weekly live contract workflow, never a pull request job) fetches exactly the pages the seeds cite, the Anthropic model pricing table and the seven OpenAI model pages (the three `gpt-5.6` siblings and, since the 2026-09-08 renewal, `gpt-5.5`, `gpt-5.5-pro`, `gpt-5.4` and `gpt-5.4-mini`), extracts each audited model's row, and compares it against the seed in both directions, write premium and long context tiers included; since RV4918 a display name matches as a whole name, because the Anthropic page lists `Claude Fable 5.1` before `Claude Fable 5` and a prefix match read the 5.1 row's cache read rate against the `claude-fable-5` seed as a drift that did not exist. The audit also judges the age of every audited row's stamp (`judgeStampAge`, `MAX_STAMP_AGE_DAYS = 60`, with a one day tolerance for a stamp authored ahead of UTC): a stamp older than sixty days fails the run even when every number still matches, and a missing, unparsable or future stamp fails it too, because a matching table with a stamp nobody renews is exactly what a host declaring `strictPricing.maxRatesAgeDays` discards on the calendar. Only the scheduled run reads the clock; a pull request job never turns red by the calendar.

The stamps as of the 2026-09-08 renewal:

| Rows | `ratesVerifiedAt` | Audited against |
|---|---|---|
| `claude-fable-5`, `claude-opus-4-8`, `claude-opus-4-7`, `claude-sonnet-5`, `claude-haiku-4-5`, `claude-opus-4-6`, `claude-sonnet-4-6` | `2026-09-08` | the model pricing table, every row re read with the audit's own extractor on 2026-09-07 and again on 2026-09-08; no rate moved, so `pricingVersion` stays `anthropic-2026-07-31` |
| `gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna` and the `gpt-5.6` alias | `2026-09-08` | each sibling's model page, re read on both days; no rate moved |
| `gpt-5.5`, `gpt-5.5-pro`, `gpt-5.4`, `gpt-5.4-mini` | `2026-09-08` | each model's own page, read with the audit's extractor for the first time on 2026-09-08 and audited weekly since: `gpt-5.5` and `gpt-5.4` document the 272K long context tier the rows lacked, and every page states a larger window than the seeds carried, so the OpenAI table moved to `pricingVersion` `openai-2026-09-08` |

The renewal ritual is a re read, a bump, and a release: re read the documented page with the audit's own extractor, bump the stamp (the constant for Anthropic, the rows for OpenAI) with `pricingVersion` unchanged when no rate moved, and ship it in a release, because a run pins the rows its settles wrote and only runs started after the release carry the renewed date; a moved rate is a different act, a new `pricingVersion` with a changeset, per the rules above. The 2026-09-07 renewal is the precedent: the Anthropic stamp sat at `2026-07-31` while the page's 2026-08-10 revision had made the Sonnet 5 rate permanent, the weekly comparison stayed green because no number moved, and a `maxRatesAgeDays` host would have discarded a correct table on the calendar; the age rule now pages for the next renewal instead of remembering it. The 2026-09-08 renewal is the precedent of the other kind: reading the four pre-5.6 pages with the extractor for the first time found a documented tier the seeds lacked, so the stamp bump and the moved rate shipped together, the rate under the new `pricingVersion`, exactly as the rules above separate them.
