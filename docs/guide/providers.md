...
A pricing seed makes two different kinds of claims, and rulvar keeps them apart. The seed's **rates** are conservative facts for bounding: admission reserves, run ceilings, and the settled fold all price under them, so a stale seed errs by refusing work early or reporting spend the provider will not actually bill, never by hiding spend. The seed's **fidelity to the provider** is a verification event with a date: every priced row carries `ratesVerifiedAt`, the ISO date it was last checked against the provider's documented rate pages or, stronger, against the provider's own billing categories (the GPT-5.6 family's `2026-07-30` is a billing confirmation: the twelfth-run statement reconciliation matched all eight per-model per-component dashboard categories to the cent).
