...
The audit also judges the age of every audited row's `ratesVerifiedAt` (RV4918): a stamp older than sixty days fails the scheduled run even when every documented number still matches, because a matching table with a stamp nobody renews is exactly what a host declaring `strictPricing.maxRatesAgeDays` discards on the calendar. The renewal is a re read of the page and a release that bumps the stamp, with `pricingVersion` unchanged when no rate moved; the 2026-09-07 Anthropic renewal is the precedent, where the Sonnet 5 rate the table called introductory had become permanent on 2026-08-10 while the numbers, and so the weekly comparison, never moved. Only the scheduled run reads the clock; a pull request job never turns red by the calendar.
