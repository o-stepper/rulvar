...
Security fixes land on the latest minor of the current major and are published as a patch. The lockstep packages share one version; `@rulvar/compat` is versioned independently.
...
- **The worker sandbox is not a security boundary.** `WorkerSandboxRunner` is a determinism and blast-radius boundary: it curates the global scope so a machine-written script cannot reach `fetch`, `process`, or `import`, and it seeds the clock and the RNG so replay is stable. It does not contain hostile code, and it is not designed to. What bounds a workflow's effects comes in layers, and they are different things: the toolset decides what is callable at all; the permission chain gates individual calls (policy, not containment); worktree isolation is a FILESYSTEM boundary only (scoped files and cwd). There is no process boundary and no network boundary in the library itself: an in-process tool's `execute` has whatever OS and network access your process has. Process and network containment is opt-in per tool through the executor seam: a tool declaring a non-`inprocess` executor dispatches through a host-registered `ToolExecutorProvider`, and `@rulvar/executor` ships reference subprocess and container adapters. The subprocess adapter scrubs ambient credentials but cannot bound filesystem or network on its own; the container adapter is where the stronger isolation holds; registering and operating either remains the host's responsibility. See [Determinism](https://docs.rulvar.com/guide/determinism) and [Isolated executor](https://docs.rulvar.com/guide/isolated-executor).
...
The packages published from this repository: `@rulvar/rulvar`, the unscoped `rulvar` pointer, `@rulvar/core`, `@rulvar/anthropic`, `@rulvar/openai`, `@rulvar/bridge-ai-sdk`, `@rulvar/store-sqlite`, `@rulvar/store-postgres`, `@rulvar/store-conformance`, `@rulvar/executor`, `@rulvar/compat`, `@rulvar/plan`, `@rulvar/planner`, `@rulvar/testing`, `@rulvar/evals`, `@rulvar/cli`, and `eslint-plugin-rulvar`.
